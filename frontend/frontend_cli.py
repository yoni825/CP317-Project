...
# - Shows menus and takes user input
# - Calls backend modules for logic and data
# -------------------------------------------------------
...
def is_admin_user(user: UserAccount) -> bool:
    """Checks if the current user is the administrator."""
    return user is not None and user.username.lower() == "admin"


def print_car_details(c: Car) -> None:
    """Pretty-print a single car."""
...
def list_available_cars(cars) -> None:
...
def run_app():
    """
    Main CLI flow for the Car Rental System.
    This is the FRONTEND that talks to the backend modules.
    """

    # -----------------------------------------------
    # 1. Load users & ensure admin exists
    # -----------------------------------------------
    ...
    # -----------------------------------------------
    # 3. Initial car inventory (sample data)
    # -----------------------------------------------
    cars = [
        Car(1, 0, 0, 5, 80.0,  True, 2022, "Toyota RAV4",    101),  # SUV, Gasoline
        Car(2, 1, 1, 2, 95.5,  True, 2018, "Ford F-150",     102),  # Truck, Dieseal
    ...
    # Load existing reservations, which will also mark some cars unavailable
    reservations = load_reservations(cars)

    # -----------------------------------------------
    # 4. Main loop: search + history + admin features
    # -----------------------------------------------
    while True:
